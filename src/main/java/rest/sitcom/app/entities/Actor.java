...
@Table(name = "actor")
@NamedQueries({
		@NamedQuery(name = "Actor.findByFirstNameMatchAny", query = "FROM Actor o WHERE LOWER(o.actor_fname) LIKE CONCAT('%', ?,'%')"),
		@NamedQuery(name = "Actor.findByLastNameMatchAny", query = "FROM Actor o WHERE LOWER(o.actor_lname) LIKE CONCAT('%', ?,'%')") })
